...
static void draw_brush(GtkWidget* widget, double x, double y)
{
	...
	// Paing to the surface, where we store our state
	cr = cairo_create(surface);
...
static void activate(GtkApplication* app, gpointer user_data)
{
	...
	g_signal_connect_after(drawing_area, "resize", G_CALLBACK(resize_cb),
			NULL);
	...
	g_signal_connect(drag, "drag-begin", G_CALLBACK(drag_begin),
			drawing_area);
	g_signal_connect(drag, "drag-update", G_CALLBACK(drag_update),
			drawing_area);
	g_signal_connect(drag, "drag-end", G_CALLBACK(drag_end),
			drawing_area);
...
int main(int argc, char* argv[])
{
	...
	app = gtk_application_new("com.github.chrinkus.gtk-ex4",
